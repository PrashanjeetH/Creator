...
then
	...
	django_conf=`pip show django`
		...
		if [ $? -eq 0 ]
		then
			echo -e "${GREEN}Django Already Installed!"
		else
      echo -e "${RED}Django couldn't found !"
      echo -e "${NC}installing django..."
			pip install django
    ...
    if [ $? -eq 0 ]
    then
      echo -e "${GREEN}Project \"$PROJECT_NAME\" Created !"
      ...
      echo -e "${GREEN}Django Web application \"$APP_NAME\" Created Successfully !"

      #Copy all the Static files and templates
      echo -e "${NC}Copying all the required static files..."
      cp -r ./../static ./${APP_NAME}
      cp -r ./../templates ./${APP_NAME}

      # Create requirements
      echo""
      echo -e "${NC}Creating requirements.txt ..."
	...
	echo -e "${RED}Please Install python and set env variables for python and pip!"
fi
